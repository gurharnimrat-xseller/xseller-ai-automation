...
for page in results.get("results", []):
    ...
    title = (
        title_list[0].get("plain_text", "No title")
        if title_list
        else "No title"
    )
    ...
    milestone = (
        milestone_obj.get("name", "Unknown") if milestone_obj else "Unknown"
    )
    ...
    owner = (
        owner_list[0].get("plain_text", "Unknown") if owner_list else "Unknown"
    )

    entry_type_obj = props.get("Entry Type", {}).get("select")
    entry_type = (
        entry_type_obj.get("name", "Unknown") if entry_type_obj else "Unknown"
    )
...
for milestone, entries in milestones.items():
    if entries:
        ...
        for i, entry in enumerate(entries, 1):
            ...
            print(
                f"   Type: {entry['type']} | Owner: {entry['owner']} | Status: {entry['status']}"
            )
...
print("\n" + "=" * 100)
print(
    f"📊 TOTAL ENTRIES: {sum(len(entries) for entries in milestones.values())}"
)
print("=" * 100)

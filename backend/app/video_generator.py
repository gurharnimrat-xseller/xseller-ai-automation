...
async def generate_video_for_post(post: Post) -> Dict[str, Any]:
    ...
    try:
        ...
        voiceover_url = await generate_voiceover(
            script_sections["full_text"], post.title
        )
        ...
        print(
            f"[video] Video generated successfully for post {post.id}, asset ID: {video_asset.id}, URL: {final_video_url}"
        )
...
def parse_video_script(script: str) -> Dict[str, str]:
    ...
    print(
        f"[video] Parsed script sections - Hook: {len(sections['hook'])} chars, Main: {len(sections['main'])} chars, Why: {len(sections['why'])} chars, CTA: {len(sections['cta'])} chars"
    )

    return sections


async def get_stock_video_clips(
    script_sections: Dict[str, str], title: str
) -> list[str]:
    """Get stock video clips from StockStack API."""
    if not STOCKSTACK_API_KEY:
        print(
            "[video] StockStack API key not configured, using placeholder video"
        )
            ...
            for endpoint in endpoints:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if video_urls:
                            print(
                                f"[video] Found {len(video_urls)} stock videos from {endpoint}"
                            )
                            break
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"[video] Eleven Labs API error {response.status_code}: {error_text}"
                )
                return ""
    ...
    with Session(engine) as session:
        ...
        for post in posts:
            try:
                ...
                print(
                    f"[video] Failed to generate video for post {post.id}: {str(e)}"
                )
                import traceback
